...
namespace DHI.Mesh
{
  ...
  public static partial class MeshBoundaryExtensions
  {
    ...
    public static IGeometry BuildBoundaryGeometry(this CMeshData mesh, bool alwaysMultiPolygon = false)
    {
      ...
      return BuildBoundaryGeometry(meshFaces, alwaysMultiPolygon);
    }
    ...
    /// </para>
    /// </summary>
    private static IGeometry BuildBoundaryGeometry(List<CMeshFace> boundaryFaces, bool alwaysMultiPolygon)
    {
    ...
    private static void CreateAddElementFaces(CMeshData meshData, List<CMeshFace>[] nodeFaces, int ielmt, bool checkAllFaces)
    {
      ...
      for (int j = 0; j < elmtNodes.Count; j++)
      {
        ...
        if (fromNode.Code > 0 && toNode.Code > 0 || checkAllFaces)
          CMeshData.AddFace(element, fromNode, toNode, nodeFaces);
      }
    }
